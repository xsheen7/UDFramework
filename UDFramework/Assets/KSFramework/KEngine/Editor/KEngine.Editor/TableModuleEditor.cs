...
namespace KEngine.Editor
{
    ...
    public class SettingModuleEditor
    {
        ...
        {
            if (!Directory.Exists(AppConfig.TableSourcePath))
            {
                Directory.CreateDirectory(AppConfig.TableSourcePath);
            }
            ...
            new KDirectoryWatcher(AppConfig.TableSourcePath, (o, args) =>
            {
            ...
            Debug.Log("[SettingModuleEditor]Watching directory: " + AppConfig.TableSourcePath);
        }

  

        [MenuItem("KEngine/TableBuild/Force Compile Table + Code")]
        public static void CompileSettings()
        {
            DoCompileSettings(true);
        }
        [MenuItem("KEngine/TableBuild/Quick Compile Table")]
        ...
        public static void DoCompileSettings(bool force = true, string forceTemplate = null, bool canCustom = true)
        {
            ...
            if (AppConfig.IsUseLuaConfig)
            {
                ...
                results = new BatchCompiler().CompileAll(AppConfig.TableSourcePath, AppConfig.ExportLuaPath, genParam,compilerParam);     
            }
            else
            {
                if (!Directory.Exists(AppConfig.ExportTsvPath))
                {
                    Directory.CreateDirectory(AppConfig.ExportTsvPath);
                }

                if (!Directory.Exists(AppConfig.ExportCSharpPath))
                {
                    Directory.CreateDirectory(AppConfig.ExportCSharpPath);
                }
                ...
                var genParam = new GenParam(){forceAll = force, genCSharpClass = true,genCodeFilePath = AppConfig.ExportCSharpPath,
                    ...
                    settingCodeIgnorePattern = AppConfig.SettingCodeIgnorePattern,nameSpace = "AppTable"};
                var compilerParam = new CompilerParam() {CanExportTsv = true, ExportTsvPath = AppConfig.ExportTsvPath, ExportLuaPath = null};
                results = new BatchCompiler().CompileAll(AppConfig.TableSourcePath, AppConfig.ExportTsvPath, genParam,compilerParam);
            }
